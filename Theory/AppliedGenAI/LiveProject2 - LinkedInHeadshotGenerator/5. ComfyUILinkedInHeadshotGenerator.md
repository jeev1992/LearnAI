...
12. Add these prompts in the `CLIP Text Encode` node
    - `Positive`: professional corporate headshot of a confident person, business attire, clean background, soft even lighting, realistic skin tones, sharp focus, friendly natural smile, upper body shot, looking at camera, shallow depth of field, subtle bokeh, crisp details, high-resolution, photorealistic, award-winning portrait photography, linkedin profile photo style
    - `Negative`: blurry, grainy, pixelated, distorted face, deformed facial features, bad anatomy, unrealistic skin texture, overexposed, underexposed, harsh shadows, wide-angle distortion, exaggerated expression, cartoon, 3d render, painting, illustration, artistic filter, selfie, casual clothing, busy background, text, watermark, extra limbs, duplicate faces
...
13. Settings for `Empty Latent Image` node
    ...
    |----------------|---------|-------|
    | **width: 1016 / height: 1024** | Resolution of generated image | Slightly taller than wide – ideal for LinkedIn headshots. Dimensions are multiples of 8 for SD compatibility. |
...
## `scheduler` – Step Distribution Curve  
*(Your setting: `karras`)*  
